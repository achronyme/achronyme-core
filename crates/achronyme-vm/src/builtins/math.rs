...
macro_rules! unary_math_fn {
    ($name:expr, $f:expr) => {
        |_vm: &VM, args: &[Value]| -> Result<Value, VmError> {
            if args.len() != 1 {
...
pub fn vm_sin() -> fn(&VM, &[Value]) -> Result<Value, VmError> {
    unary_math_fn!("sin", f64::sin)
}

pub fn vm_cos() -> fn(&VM, &[Value]) -> Result<Value, VmError> {
    unary_math_fn!("cos", f64::cos)
}

pub fn vm_tan() -> fn(&VM, &[Value]) -> Result<Value, VmError> {
    unary_math_fn!("tan", f64::tan)
}

pub fn vm_asin() -> fn(&VM, &[Value]) -> Result<Value, VmError> {
    unary_math_fn!("asin", f64::asin)
}

pub fn vm_acos() -> fn(&VM, &[Value]) -> Result<Value, VmError> {
    unary_math_fn!("acos", f64::acos)
}

pub fn vm_atan() -> fn(&VM, &[Value]) -> Result<Value, VmError> {
    unary_math_fn!("atan", f64::atan)
}

pub fn vm_atan2(_vm: &VM, args: &[Value]) -> Result<Value, VmError> {
    if args.len() != 2 {
        return Err(VmError::Runtime(format!(
...
pub fn vm_sinh() -> fn(&VM, &[Value]) -> Result<Value, VmError> {
    unary_math_fn!("sinh", f64::sinh)
}

pub fn vm_cosh() -> fn(&VM, &[Value]) -> Result<Value, VmError> {
    unary_math_fn!("cosh", f64::cosh)
}

pub fn vm_tanh() -> fn(&VM, &[Value]) -> Result<Value, VmError> {
...
pub fn vm_exp() -> fn(&VM, &[Value]) -> Result<Value, VmError> {
    unary_math_fn!("exp", f64::exp)
}

pub fn vm_ln() -> fn(&VM, &[Value]) -> Result<Value, VmError> {
    unary_math_fn!("ln", f64::ln)
}

pub fn vm_log() -> fn(&VM, &[Value]) -> Result<Value, VmError> {
    binary_math_fn!("log", |x: f64, base: f64| x.log(base))
}

pub fn vm_log10() -> fn(&VM, &[Value]) -> Result<Value, VmError> {
    unary_math_fn!("log10", f64::log10)
}

pub fn vm_log2() -> fn(&VM, &[Value]) -> Result<Value, VmError> {
...
pub fn vm_floor() -> fn(&VM, &[Value]) -> Result<Value, VmError> {
    unary_math_fn!("floor", f64::floor)
}

pub fn vm_ceil() -> fn(&VM, &[Value]) -> Result<Value, VmError> {
    unary_math_fn!("ceil", f64::ceil)
}

pub fn vm_round() -> fn(&VM, &[Value]) -> Result<Value, VmError> {
    unary_math_fn!("round", f64::round)
}

pub fn vm_trunc() -> fn(&VM, &[Value]) -> Result<Value, VmError> {
...
pub fn vm_sqrt() -> fn(&VM, &[Value]) -> Result<Value, VmError> {
    unary_math_fn!("sqrt", f64::sqrt)
}

pub fn vm_abs() -> fn(&VM, &[Value]) -> Result<Value, VmError> {
    unary_math_fn!("abs", f64::abs)
}

pub fn vm_pow() -> fn(&VM, &[Value]) -> Result<Value, VmError> {
    binary_math_fn!("pow", f64::powf)
}

pub fn vm_min(_vm: &VM, args: &[Value]) -> Result<Value, VmError> {
    if args.is_empty() {
...
pub fn vm_max(_vm: &VM, args: &[Value]) -> Result<Value, VmError> {
    if args.is_empty() {
        return Err(VmError::Runtime(
...
pub fn vm_sign() -> fn(&VM, &[Value]) -> Result<Value, VmError> {
...
pub fn vm_deg() -> fn(&VM, &[Value]) -> Result<Value, VmError> {
    unary_math_fn!("deg", f64::to_degrees)
}

pub fn vm_rad() -> fn(&VM, &[Value]) -> Result<Value, VmError> {
    unary_math_fn!("rad", f64::to_radians)
}

pub fn vm_cbrt() -> fn(&VM, &[Value]) -> Result<Value, VmError> {
...
pub fn vm_pi(_vm: &VM, args: &[Value]) -> Result<Value, VmError> {
    if !args.is_empty() {
        return Err(VmError::Runtime(format!(
...
pub fn vm_e(_vm: &VM, args: &[Value]) -> Result<Value, VmError> {
    if !args.is_empty() {
        return Err(VmError::Runtime(format!(
...
pub fn vm_set_precision(vm: &VM, args: &[Value]) -> Result<Value, VmError> {
    if args.len() != 1 {
        return Err(VmError::Runtime(format!(
